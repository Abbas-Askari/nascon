  ...
  try {
    ...
    return jwt.sign(
      ...
      (error, token) => {
        if (error)
          return res.json({
            errors: [{ path: "custom", message: "Can not sign up" }],
          });
...
async function login(req, res) {
  ...
  if (!existing)
    return res.json({
      errors: [{ path: "email", message: "No User with given Email" }],
    });

  console.log("chacking", existing.password, user.password);
  ...
  console.log("signing");
  jwt.sign({ _id: existing._id }, process.env.JWT_SECRET, (error, token) => {
    if (error)
      return res.json({
        errors: [{ path: "custom", message: "Can not Login" }],
      });
